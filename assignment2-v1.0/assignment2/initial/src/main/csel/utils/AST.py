...
class ConstDecl(Decl):
    constant: Id
    constDimen: List[int]       # empty list for scalar variable
    typ: Type                   # None type if empty
    constInit: Literal
    ...
    def __str__(self):
        initial = ("," + str(self.varInit)) if self.varInit else ""
        dimen = ("," + printlist(self.varDimen)) if self.varDimen else ""
        return "ConstDecl(" + str(self.variable) + dimen + initial + ")"
...
class JSONLiteral(Literal):
    ...
    def __str__(self):
        return "JSONLiteral(" + ','.join(["(" + str(x[0]) + "," + str[1] + ")" for x in self.value])
